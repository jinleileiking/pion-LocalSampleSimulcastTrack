...
// LocalSampleSimulcastTrack is a local track that simplifies writing samples.
...
type LocalSampleSimulcastTrack struct {
...
type LocalSampleSimulcastTrackOptions func(s *LocalSampleSimulcastTrack)

func WithRTCPHandler(cb func(rtcp.Packet)) LocalSampleSimulcastTrackOptions {
	return func(s *LocalSampleSimulcastTrack) {
		s.onRTCP = cb
	}
}

func NewLocalSimulcastSampleTrack(c webrtc.RTPCodecCapability, simulcastID, rid string, opts ...LocalSampleSimulcastTrackOptions) (*LocalSampleSimulcastTrack, error) {
	s := &LocalSampleSimulcastTrack{}
	s.rid = rid
	s.simulcastID = simulcastID
	for _, o := range opts {
		o(s)
	}
	rtpTrack, err := webrtc.NewTrackLocalStaticRTP(c, s.simulcastID, s.simulcastID, webrtc.WithRTPStreamID(s.rid))
	if err != nil {
		return nil, err
	}
	s.rtpTrack = rtpTrack
...
func (s *LocalSampleSimulcastTrack) SetTransceiver(transceiver *webrtc.RTPTransceiver) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *LocalSampleSimulcastTrack) ID() string { return s.rtpTrack.ID() }

// RID is the RTP stream identifier.
func (s *LocalSampleSimulcastTrack) RID() string {
	return s.rtpTrack.RID()
}

// StreamID is the group this track belongs too. This must be unique
func (s *LocalSampleSimulcastTrack) StreamID() string { return s.rtpTrack.StreamID() }

// Kind controls if this TrackLocal is audio or video
func (s *LocalSampleSimulcastTrack) Kind() webrtc.RTPCodecType { return s.rtpTrack.Kind() }

// Codec gets the Codec of the track
// func (s *LocalSampleSimulcastTrack) Codec() webrtc.RTPCodecCapability {
// 	return s.rtpTrack.Codec()
// }

// func (s *LocalSampleSimulcastTrack) IsBound() bool {
// 	return s.bound.Load()
// }

// Bind is an interface for TrackLocal, not for external consumption
func (s *LocalSampleSimulcastTrack) Bind(t webrtc.TrackLocalContext) (webrtc.RTPCodecParameters, error) {
...
func (s *LocalSampleSimulcastTrack) Unbind(t webrtc.TrackLocalContext) error {
	s.lock.Lock()
	onUnbind := s.onUnbind
	s.bound.Store(false)
...
// func (s *LocalSampleSimulcastTrack) StartWrite(provider SampleProvider, onComplete func()) error {
// 	s.lock.Lock()
// 	defer s.lock.Unlock()
...
// func (s *LocalSampleSimulcastTrack) OnBind(f func()) {
// 	s.lock.Lock()
// 	s.onBind = f
// 	s.lock.Unlock()
// }
...
// func (s *LocalSampleSimulcastTrack) OnUnbind(f func()) {
// 	s.lock.Lock()
// 	s.onUnbind = f
// 	s.lock.Unlock()
// }

func (s *LocalSampleSimulcastTrack) WriteSample(sample media.Sample, opts *SampleWriteOptions) error {
	s.lock.RLock()
	p := s.packetizer
...
func (s *LocalSampleSimulcastTrack) rtcpWorker(rtcpReader interceptor.RTCPReader) {
...
// func (s *LocalSampleSimulcastTrack) writeWorker(provider SampleProvider, onComplete func()) {
